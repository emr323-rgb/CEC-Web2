...
import { staff, locationTreatments, treatments } from './shared/schema';
import { eq } from 'drizzle-orm';

async function main() {
  console.log('Starting to add location-specific data...');
  ...
  if (existingTreatmentLinks.length === 0) {
    ...
    if (allTreatments.length > 0) {
      ...
      console.log('No treatments found to associate with location');
    }
  }
...
main().catch(e => {
  console.error('Error during data creation:', e);
  process.exit(1);
});